...
export const getUserProfilerController = async (req, res) => {
  try {
    const response = await getUserByIdService({ _id: req.user.id });
    const user = responseContentValidator(response);
    return successProfiler(res, 200, "getUserProfilerController", { user });
  } catch (error) {
    errorProfiler(error, res, "getUserProfilerController");
  }
};
