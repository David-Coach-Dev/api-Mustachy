...
// Imports
//=============
import { deleteImage, setFileNameImage, uploadImage } from "#lib/cloudinary";
import { validateKeysInMongooseModel } from "#lib/validations";
import { URL } from "url";
...
const validateMimeType = (file) => {
  const resourceType = CLOUDINARY_RESOURCE_TYPE;
  const allowedExtensions = CLOUDINARY_RESOURCE_TYPE_EXT.replace(
    /[[\]]/g,
    ""
  ).split(",");
...
const validateSize = (file) => {
  const maxSize = parseInt(CLOUDINARY_MAX_FILE_SIZE, 10);
  const minSize = 1; // 0B
...
const getDefaultImage = async (route) => {
  ...
  console.log("modelNam->", modelName);
